...
const {definition,query,resolver}  = schema('Historial','Records');
module.exports = {
  definition,
  query,
  resolver: {
    Query : {
      [resolver]: async (obj,{start,limit,filters},{context}) => {
        ...
        console.log(filters);
        const startIndex = parseInt(start,10)>=0 ? parseInt(start,10) :0;
        const query = await resolverFilters(filters,'Historial',{mostrar:true});
        console.log(query);
        const {totalCount,edges,pageInfo} = await petition.historial(query,startIndex,limit);
        return {
          totalCount,
          edges,
          pageInfo,
